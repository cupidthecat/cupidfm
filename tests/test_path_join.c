...
// (Same as utils.c but without ncurses dependencies)
...
// Test empty path handling (critical fix #10)
bool test_path_join_empty_path_safety() {
    char result[MAX_PATH_LENGTH];
    const char *empty_path = "";
    size_t path_len = strlen(empty_path);
    
    // The fix ensures we check path_len == 0 before accessing path[path_len - 1]
    // This test verifies the fix works
    path_join(result, empty_path, "test");
    ASSERT_STR_EQ(result, "test", "Should handle empty path safely");
    
    return true;
}

int main() {
    printf("=== Path Join Tests ===\n\n");
    ...
    RUN_TEST(test_path_join_empty_path_safety);
