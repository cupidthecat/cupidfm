...
// Register common Ctrl+Arrow escape sequences so wgetch() returns our
// CTRL_*_CODE. Call once after initscr()/keypad() (safe to call multiple
// times).
static void register_ctrl_arrow_keys_once(void) {
  ...
  // Most terminals (xterm, alacritty, kitty, wezterm, iTerm2, GNOME Terminal,
  // etc.)
...
// Enable/disable terminal mouse tracking for drag motion (works in most
// xterm-like terms).
...
static long compute_directory_size_full_progress(const char *dir_path,
                                                 const char *job_path);
...
bool dir_size_can_enqueue(void) { return dir_size_user_idle(); }

static DirSizeEntry *dir_size_find_entry(const char *path) {
  for (DirSizeEntry *entry = dir_size_cache_head; entry != NULL;
       entry = entry->next) {
...
static long compute_directory_size_full(const char *dir_path) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (S_ISDIR(statbuf.st_mode)) {
      ...
      if (dir_size == DIR_SIZE_VIRTUAL_FS ||
          dir_size == DIR_SIZE_PERMISSION_DENIED) {
        closedir(dir);
        return dir_size;
...
static bool g_editor_dirty_clear_requested = false;
static bool g_editor_close_requested = false;
static bool g_editor_reload_requested = false;
static bool g_editor_readonly = false;

static bool editor_block_if_readonly(WINDOW *notification_window,
                                     struct timespec *last_notif_check) {
  if (!g_editor_readonly)
    return false;
  if (notification_window) {
    werase(notification_window);
    mvwprintw(notification_window, 0, 0, "Editor is read-only");
...
static bool editor_load_file_into_buffer(const char *path, TextBuffer *buf) {
  if (!path || !*path || !buf)
    return false;
  ...
  if (!buf->lines) {
    buf->capacity = 100;
    buf->lines = malloc(sizeof(char *) * (size_t)buf->capacity);
    if (!buf->lines) {
      fclose(rf);
      return false;
    }
  ...
  if (is_empty) {
    if (buf->num_lines >= buf->capacity) {
      buf->capacity = (buf->capacity > 0) ? (buf->capacity * 2) : 2;
      char **tmp = realloc(buf->lines, sizeof(char *) * (size_t)buf->capacity);
      if (!tmp)
        return false;
      buf->lines = tmp;
...
static EditorSnapshot editor_snapshot_make(const TextBuffer *buf,
                                           ...
                                           int start_line) {
  EditorSnapshot s = {0};
  if (!buf || !buf->lines || buf->num_lines < 0)
    return s;
...
static void editor_stack_push(EditorSnapshot *stk, int *len,
                              EditorSnapshot *snap) {
  if (!stk || !len || !snap || !snap->lines)
    return;
...
static bool editor_stack_pop(EditorSnapshot *stk, int *len,
                             EditorSnapshot *out) {
  if (!stk || !len || !out)
    return false;
  if (*len <= 0)
...
static void editor_apply_snapshot(TextBuffer *buf, EditorSnapshot *snap,
  ...
  buf->capacity = (snap->capacity > 0)
                      ? snap->capacity
                      : ((snap->num_lines > 0) ? snap->num_lines : 1);
...
static void editor_undo_record(UndoManager *um, TextBuffer *buf,
  ...
  EditorSnapshot s =
      editor_snapshot_make(buf, cursor_line, cursor_col, start_line);
  if (!s.lines)
    return;
...
static bool editor_do_undo(UndoManager *um, TextBuffer *buf, int *cursor_line,
                           int *cursor_col, int *start_line) {
  if (!um || !buf)
    return false;

  EditorSnapshot prev;
  if (!editor_stack_pop(um->undo, &um->undo_len, &prev))
    return false;

  EditorSnapshot cur = editor_snapshot_make(buf, cursor_line ? *cursor_line : 0,
                                            cursor_col ? *cursor_col : 0,
                                            start_line ? *start_line : 0);
  if (cur.lines)
    editor_stack_push(um->redo, &um->redo_len, &cur);

  editor_apply_snapshot(buf, &prev, cursor_line, cursor_col, start_line);
  return true;
}

static bool editor_do_redo(UndoManager *um, TextBuffer *buf, int *cursor_line,
                           int *cursor_col, int *start_line) {
  if (!um || !buf)
    return false;

  EditorSnapshot next;
  if (!editor_stack_pop(um->redo, &um->redo_len, &next))
    return false;
  ...
  EditorSnapshot cur = editor_snapshot_make(buf, cursor_line ? *cursor_line : 0,
                                            ...
                                            start_line ? *start_line : 0);
  if (cur.lines)
    editor_stack_push(um->undo, &um->undo_len, &cur);
...
static void normalize_selection(int *s_line, int *s_col, int *e_line,
                                int *e_col) {
...
static bool editor_mouse_to_buffer_pos(WINDOW *window, TextBuffer *buffer,
                                       int start_line, int mouse_y, int mouse_x,
                                       int *out_line, int *out_col) {
  if (!window || !buffer || !buffer->lines || !out_line || !out_col)
    return false;
  ...
  if (rel_y < 1 || rel_y >= max_y - 1)
    return false;
  if (rel_x < 1 || rel_x >= max_x - 1)
    return false;
  ...
  if (line_index < 0 || line_index >= buffer->num_lines)
    return false;
...
static char *copy_selection_or_line(TextBuffer *buffer, int cursor_line,
                                    int cursor_col) {
  ...
  if (g_sel_active && !selection_is_empty()) {
    normalize_selection(&s_line, &s_col, &e_line, &e_col);
    if (s_line >= buffer->num_lines)
      return NULL;
    if (e_line >= buffer->num_lines)
      e_line = buffer->num_lines - 1;
  ...
  if (cursor_line < 0 || cursor_line >= buffer->num_lines)
    return NULL;
  const char *line =
      buffer->lines[cursor_line] ? buffer->lines[cursor_line] : "";
...
static void delete_selection(TextBuffer *buffer) {
  if (!buffer || !buffer->lines || !g_sel_active || selection_is_empty())
    return;
  ...
  memcpy(merged + start_keep, end_line + end_keep_from,
         (size_t)(end_len - end_keep_from));
  merged[new_len - 1] = '\0';
...
static void editor_selection_to_uppercase(TextBuffer *buffer) {
  ...
  for (int i = s_line; i <= e_line; i++) {
    ...
    int end = (i == e_line)
                  ? MIN(e_col, len)
                  : len; // end is EXCLUSIVE (matches your copy/delete logic)
    if (end < start)
      end = start;
...
static void insert_text_at_cursor(TextBuffer *buffer, int *cursor_line,
                                  int *cursor_col, const char *text) {
  if (!buffer || !buffer->lines || !text || !cursor_line || !cursor_col)
    return;
  ...
  if (parts == 1) {
    ...
    char *new_line =
        realloc(buffer->lines[*cursor_line], (size_t)line_len + insert_len + 1);
    if (!new_line)
      return;
    memmove(new_line + col + insert_len, new_line + col,
            (size_t)(line_len - col + 1));
...
char *editor_get_content_copy(void) {
  if (!is_editing || !g_editor_buffer || !g_editor_buffer->lines)
    return NULL;

  size_t total = 0;
  for (int i = 0; i < g_editor_buffer->num_lines; i++) {
    const char *line =
        g_editor_buffer->lines[i] ? g_editor_buffer->lines[i] : "";
    total += strlen(line);
...
char *editor_get_line_copy(int line_num) {
  if (!is_editing || !g_editor_buffer || !g_editor_buffer->lines)
    return NULL;
  if (line_num <= 0 || line_num > g_editor_buffer->num_lines)
    return NULL;
...
bool editor_get_cursor(int *line, int *col) {
  if (!is_editing)
    return false;
  if (line)
    *line = g_editor_cursor_line + 1; // Convert to 1-indexed
  if (col)
    *col = g_editor_cursor_col + 1; // Convert to 1-indexed
  return true;
}
...
bool editor_get_selection(int *start_line, int *start_col, int *end_line,
  ...
  if (start_line)
    *start_line = sl + 1; // Convert to 1-indexed
  if (start_col)
    *start_col = sc + 1; // Convert to 1-indexed
  if (end_line)
    *end_line = el + 1; // Convert to 1-indexed
  if (end_col)
    *end_col = ec + 1; // Convert to 1-indexed
  return true;
}

bool editor_insert_text(const char *text) {
  if (!is_editing || !g_editor_buffer || !text)
    return false;

  int line = g_editor_cursor_line;
  int col = g_editor_cursor_col;

  if (line < 0 || line >= g_editor_buffer->num_lines)
    return false;
  ...
  while (*p) {
    if (*p == '\n') {
      ...
      if (insert_len > 0 || p == line_start) {
        ...
        memcpy(new_line + col + insert_len, curr_line + col,
               curr_len - col + 1);
      ...
      if (g_editor_buffer->num_lines >= g_editor_buffer->capacity) {
        ...
        char **tmp = realloc(g_editor_buffer->lines,
                             sizeof(char *) * g_editor_buffer->capacity);
        if (!tmp) {
          free(rest);
          return false;
        }
      ...
      memmove(&g_editor_buffer->lines[line + 2],
              &g_editor_buffer->lines[line + 1],
...
bool editor_replace_text(int start_line, int start_col, int end_line,
  ...
  if (start_line < 0 || start_line >= g_editor_buffer->num_lines)
    return false;
  if (end_line < 0 || end_line >= g_editor_buffer->num_lines)
    return false;
  ...
  if (start_line == end_line) {
    ...
    memmove(&g_editor_buffer->lines[start_line + 1],
            &g_editor_buffer->lines[end_line + 1],
...
bool editor_save_current(struct PluginManager *pm) {
  ...
  FILE *file = fopen(g_editor_path, "w");
  if (!file)
    return false;
  ...
  g_editor_dirty_clear_requested = true;

  if (pm) {
    plugins_notify_editor_save(pm, g_editor_path);
  }
...
bool editor_save_as(struct PluginManager *pm, const char *path) {
  ...
  if (path[0] != '/' && g_editor_path[0]) {
    // Treat relative Save As paths as relative to the current editor file's
    // directory (matches typical editor behavior and the file manager cwd).
    const char *slash = strrchr(g_editor_path, '/');
    if (slash && slash != g_editor_path) {
      size_t dir_len = (size_t)(slash - g_editor_path);
      if (dir_len + 1 + strlen(path) + 1 > sizeof(full_path))
        return false;
      memcpy(full_path, g_editor_path, dir_len);
      full_path[dir_len] = '\0';
      if (snprintf(full_path + dir_len, sizeof(full_path) - dir_len, "/%s",
                   path) >= (int)(sizeof(full_path) - dir_len)) {
        return false;
      }
      save_path = full_path;
    }
  ...
  // Adopt the new path (so future saves go to the new location).
  strncpy(g_editor_path, save_path, sizeof(g_editor_path) - 1);
  ...
  if (pm) {
    // Ensure the file manager view picks up newly created files after returning
    // from the editor.
    plugins_request_reload(pm);

    // Try to select the new filename in the directory view (best effort).
    const char *base = strrchr(g_editor_path, '/');
    base = base ? (base + 1) : g_editor_path;
    if (base && *base) {
      plugins_request_select(pm, base);
    }
...
bool editor_request_reload(void) {
  if (!is_editing || !g_editor_path[0])
    return false;
  g_editor_reload_requested = true;
  return true;
}
...
bool editor_set_readonly(bool readonly) {
  if (!is_editing)
    return false;
  g_editor_readonly = readonly ? true : false;
  return true;
}
...
static void dir_size_progress_maybe_update(DirSizeProgressCtx *ctx) {
  if (!ctx || !ctx->job_path || !ctx->total)
    return;

  // Throttle updates: at most ~20Hz or every N entries to keep mutex overhead
  // low.
...
static long
compute_directory_size_full_progress_inner(const char *dir_path,
...
// Recursive function to calculate directory size with guard rails to keep UI
...
void display_file_info(WINDOW *window, const char *file_path, int max_x) {
  ...
  // Attempt to retrieve file statistics (use lstat for POSIX-correct symlink
  // handling)
  ...
  if (S_ISLNK(file_stat.st_mode)) {
    ...
    ssize_t target_len =
        readlink(file_path, link_target, sizeof(link_target) - 1);
    if (target_len > 0) {
      ...
      if (stat(file_path, &target_stat) == 0) {
        if (S_ISDIR(target_stat.st_mode)) {
          ...
          mvwprintw(window, 1, 2, "Symlink -> %s (file, %ld bytes)",
                    link_target, (long)target_stat.st_size);
        }
      } else {
  ...
  if (S_ISDIR(file_stat.st_mode)) {
    ...
    long elapsed_ns =
        (now.tv_sec - last_size_path_change.tv_sec) * 1000000000L +
        (now.tv_nsec - last_size_path_change.tv_nsec);
    bool allow_enqueue =
        (elapsed_ns >= DIR_SIZE_REQUEST_DELAY_NS) && dir_size_can_enqueue();
    ...
    if (dir_size == -1) {
      ...
      if (p > 0) {
        ...
        format_dir_size_pending_animation(fileSizeStr, sizeof(fileSizeStr),
                                          path_changed);
      } else {
    ...
    mvwprintw(window, 2, 2, "%-*s %s", label_width,
              "📁 Directory Size:", fileSizeStr);
  } else {
    char fileSizeStr[20];
    format_file_size(fileSizeStr, file_stat.st_size);
    mvwprintw(window, 2, 2, "%-*s %s", label_width,
              "📏 File Size:", fileSizeStr); // Updated with emoji
  }
  // Display MIME type using libmagic
  magic_t magic_cookie =
      magic_open(MAGIC_MIME_TYPE | MAGIC_SYMLINK | MAGIC_CHECK);
  if (magic_cookie == NULL) {
    mvwprintw(window, 5, 2, "%-*s %s", label_width,
              "📂 MIME type:", "Error initializing magic library");
    return;
  }
  if (magic_load(magic_cookie, NULL) != 0) {
    mvwprintw(window, 5, 2, "%-*s %s", label_width,
              "📂 MIME type:", magic_error(magic_cookie));
  ...
  if (mime_type == NULL) {
    mvwprintw(window, 5, 2, "%-*s %s", label_width,
              "📂 MIME type:", "Unknown (error)");
  } else {
    size_t value_width =
        (size_t)(max_x - 2 - label_width - 1); // 2 for left margin, 1 for space
    ...
    mvwprintw(window, 5, 2, "%-*s %s %s", label_width, emoji,
              "MIME type:", display_mime);
  }
  magic_close(magic_cookie);
}
/**
 ...
 */
void render_text_buffer(WINDOW *window, TextBuffer *buffer, int *start_line,
                        int cursor_line, int cursor_col) {
  if (!buffer || !buffer->lines) {
    return;
  }
  ...
  // IMPORTANT: *start_line may have changed above; compute comment state using
  // FINAL start_line.
  ...
  // Subtract: left border (1) + line numbers + separator (1) + right border (1)
  // + padding (1)
  int content_width = max_x - label_width - 4;
  ...
  const int scroll_margin =
      5; // Number of columns to keep visible on either side
  ...
  // Reset horizontal scroll if window got wider and we can now show more
  // content This ensures we use all available horizontal space
  else if (content_width > last_content_width && h_scroll > 0) {
    // Find the longest line in the visible area
    int max_line_length = 0;
    for (int i = 0; i < content_height && (*start_line + i) < buffer->num_lines;
         i++) {
      const char *line =
          buffer->lines[*start_line + i] ? buffer->lines[*start_line + i] : "";
    ...
    if (max_line_length <= content_width) {
      ...
      // Otherwise, reduce scroll to show as much as possible while keeping
      // cursor visible If cursor is within the new wider view, we can reduce
      // scroll
  ...
  // If cursor has plenty of room, reduce scroll to show more content to the
  // left Only do this if content_width is reasonable
  if (content_width >= scroll_margin * 2 && h_scroll > 0 &&
      cursor_col < h_scroll + content_width - scroll_margin * 2) {
    // Find the longest line in the visible area to see how much we need to
    // scroll
  ...
  for (int i = 0; i < content_height && (*start_line + i) < buffer->num_lines;
    ...
    // even if the line is shorter than h_scroll (important for /** and */
    // lines)
    if (h_scroll < line_length) {
      ...
      syntax_highlight_line(window, visible_line, current_syntax,
                            &in_block_comment, i + 1, content_start,
                            content_width, buffer->lines, buffer->num_lines,
                            current_line_index);
    } else {
      // Line is shorter than h_scroll, but we still need to process it for
      // block comment state Pass empty string but still update state based on
      // the full line
      syntax_highlight_line(window, "", current_syntax, &in_block_comment,
                            i + 1, content_start, content_width, buffer->lines,
                            buffer->num_lines, current_line_index);
    }
    ...
    // If this is the cursor line, draw a visible cursor (even on
    // empty/whitespace)
    if ((*start_line + i) == cursor_line) {
      ...
      if (!at_eol)
        ch_under = (unsigned char)line[cc];

      // If we're at EOL or on whitespace, ensure the cell contains a plain
      // space, then reverse it.
...
static int parse_ctrl_arrow_seq(WINDOW *win) {
  ...
  if (mod == 5 || mod == 6) {
    if (final == 'A')
      return CTRL_UP_CODE;
    if (final == 'B')
      return CTRL_DOWN_CODE;
    if (final == 'C')
      return CTRL_RIGHT_CODE;
    if (final == 'D')
      return CTRL_LEFT_CODE;
  }
...
static inline bool is_ws_char(char c) { return (c == ' ' || c == '\t'); }
...
static int word_left_col(const char *line, int col) {
  ...
  col--;
  while (col > 0 && is_ws_char(line[col]))
    col--;
  while (col > 0 && !is_ws_char(line[col - 1]))
    col--;
  return col;
}
...
static int word_right_col(const char *line, int col) {
  ...
  if (col < len) {
    while (col < len && !is_ws_char(line[col]))
      col++;
    while (col < len && is_ws_char(line[col]))
      col++;
  }
...
void edit_file_in_terminal(WINDOW *window, const char *file_path,
                           WINDOW *notification_window, KeyBindings *kb,
  ...
  // Create a full-screen editor window (full terminal width, minus banner and
  // notification) This gives us maximum horizontal space for editing The editor
  // window is used for displaying and editing file content
  ...
  // --- Make Ctrl+C / Ctrl+V come through as bytes (3 / 22) instead of
  // signals/paste handling.
  raw();
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    if (text_buffer.num_lines >= text_buffer.capacity) {
      ...
      char **tmp =
          realloc(text_buffer.lines, sizeof(char *) * text_buffer.capacity);
      if (!tmp) {
        mvwprintw(notification_window, 1, 2, "Memory allocation error");
        wrefresh(notification_window);
        fclose(file);
  ...
  render_text_buffer(editor_window, &text_buffer, &start_line, cursor_line,
                     cursor_col);
  ...
  clock_gettime(CLOCK_MONOTONIC, &last_notif_check);
  int total_scroll_length = COLS + (BANNER_TEXT ? strlen(BANNER_TEXT) : 0) +
                            (BUILD_INFO ? strlen(BUILD_INFO) : 0) + 4;

  while (!exit_edit_mode) {
    /* NEW: Track old cursor position for cursor move detection */
    int old_cursor_line = cursor_line;
    int old_cursor_col = cursor_col;

    if (g_editor_reload_requested) {
      g_editor_reload_requested = false;
      bool dirty_now = editor_dirty || g_editor_dirty;
      if (dirty_now && !confirm_discard_unsaved(editor_window)) {
        if (notification_window) {
          werase(notification_window);
          mvwprintw(notification_window, 0, 0, "Reload canceled");
          wrefresh(notification_window);
        }
        render_text_buffer(editor_window, &text_buffer, &start_line, cursor_line,
                           cursor_col);
      ...
      if (!editor_load_file_into_buffer(g_editor_path, &text_buffer)) {
        if (notification_window) {
          werase(notification_window);
          mvwprintw(notification_window, 0, 0, "Reload failed: %s", g_editor_path);
          wrefresh(notification_window);
        }
        render_text_buffer(editor_window, &text_buffer, &start_line, cursor_line,
                           cursor_col);
      ...
      // Reopen editor file handle (best effort).
      if (file) {
        fclose(file);
        file = NULL;
      }
      file = fopen(g_editor_path, "r+");
      if (!file) {
        file = fopen(g_editor_path, "r");
      }

      // Reset view/cursor state.
      cursor_line = 0;
      cursor_col = 0;
      start_line = 0;
      g_sel_active = false;
      ...
      if (notification_window) {
        werase(notification_window);
        mvwprintw(notification_window, 0, 0, "Reloaded: %s", g_editor_path);
        wrefresh(notification_window);
      }
    ...
    if (g_editor_close_requested) {
      g_editor_close_requested = false;
      bool dirty_now = editor_dirty || g_editor_dirty;
      if (!dirty_now || confirm_discard_unsaved(editor_window)) {
        exit_edit_mode = true;
        break;
    ...
    if (resized) {
      ...
      if (editor_window) {
        ...
        werase(editor_window);
        box(editor_window, 0, 0);
        render_text_buffer(editor_window, &text_buffer, &start_line,
      ...
      total_scroll_length = COLS + (BANNER_TEXT ? strlen(BANNER_TEXT) : 0) +
                            (BUILD_INFO ? strlen(BUILD_INFO) : 0) + 4;
    }
    ...
    if (ch == ERR) {
      ...
      long banner_time_diff =
          (current_time.tv_sec - last_banner_update.tv_sec) * 1000000 +
          (current_time.tv_nsec - last_banner_update.tv_nsec) / 1000;
      if (banner_time_diff >= BANNER_SCROLL_INTERVAL && BANNER_TEXT &&
          bannerwin) {
        pthread_mutex_lock(&banner_mutex);
        draw_scrolling_banner(bannerwin, BANNER_TEXT, BUILD_INFO,
                              banner_offset);
        pthread_mutex_unlock(&banner_mutex);
      ...
      long notif_time_diff =
          (current_time.tv_sec - last_notif_check.tv_sec) * 1000 +
      ...
      if (!should_clear_notif && can_clear_notif &&
          notif_time_diff >= NOTIFICATION_TIMEOUT_MS && notifwin) {
        pthread_mutex_lock(&banner_mutex);
        werase(notifwin);
        wrefresh(notifwin);
        pthread_mutex_unlock(&banner_mutex);
    ...
    if (g_editor_dirty_clear_requested) {
      editor_dirty = false;
      g_editor_dirty_clear_requested = false;
    }
    if (editor_dirty)
      g_editor_dirty = true;
    if (g_editor_dirty)
    ...
    if (pm && plugins_handle_key(pm, plugin_ch)) {
      should_clear_notif = false;
      // Reset notification timer so it stays visible for the normal timeout
      // period
      clock_gettime(CLOCK_MONOTONIC, &last_notif_check);

      if (g_editor_close_requested) {
        // Defer the actual close handling to the top of the next loop iteration
        // (so we keep editor loop cleanup in one place).
        continue;
      }
    ...
    // 2) Save file
      ...
      if (editor_block_if_readonly(notification_window, &last_notif_check))
        continue;
      fclose(file);
      file = fopen(g_editor_path, "w");
      if (!file) {
      ...
      file = fopen(g_editor_path, "r+");
      if (!file) {
        mvwprintw(notification_window, 0, 0,
                  "Error reopening file for read+write");
        wrefresh(notification_window);
      }

      werase(notification_window);
      mvwprintw(notification_window, 0, 0, "File saved: %s", g_editor_path);
      wrefresh(notification_window);
      editor_dirty = false;
      g_editor_dirty = false;
    ...
    // Ctrl+C (copy)
      ...
      char *copied =
          copy_selection_or_line(&text_buffer, cursor_line, cursor_col);
      if (copied) {
        free(g_editor_clipboard);
        g_editor_clipboard = copied;
        if (notification_window) {
          werase(notification_window);
          mvwprintw(notification_window, 0, 0, "Copied %zu bytes",
                    strlen(g_editor_clipboard));
          wrefresh(notification_window);
          should_clear_notif = false;
          clock_gettime(CLOCK_MONOTONIC, &last_notif_check);
        }
    ...
    // Ctrl+A (select all)
      ...
      if (text_buffer.num_lines <= 0) {
        ...
        const char *last = text_buffer.lines[g_sel_end_line]
                               ? text_buffer.lines[g_sel_end_line]
                               : "";
    ...
    // Ctrl+Z (undo)
      ...
      if (editor_block_if_readonly(notification_window, &last_notif_check))
        continue;
      if (editor_do_undo(&um, &text_buffer, &cursor_line, &cursor_col,
                         &start_line)) {
        editor_dirty = true;
        if (notification_window) {
          werase(notification_window);
    ...
    // Ctrl+Y (redo)
      ...
      if (editor_block_if_readonly(notification_window, &last_notif_check))
        continue;
      if (editor_do_redo(&um, &text_buffer, &cursor_line, &cursor_col,
                         &start_line)) {
        editor_dirty = true;
        if (notification_window) {
          werase(notification_window);
    ...
    // Ctrl+X (cut)
      ...
      if (copied) {
        ...
        if (notification_window) {
          werase(notification_window);
          mvwprintw(notification_window, 0, 0, "Cut %zu bytes",
                    strlen(g_editor_clipboard));
          wrefresh(notification_window);
          should_clear_notif = false;
          clock_gettime(CLOCK_MONOTONIC, &last_notif_check);
      ...
      if (g_sel_active && !selection_is_empty()) {
        ...
        if (cursor_line >= 0 && cursor_line < text_buffer.num_lines) {
          ...
          if (text_buffer.num_lines <= 0) {
            ...
            cursor_col =
                MIN(cursor_col, (int)strlen(text_buffer.lines[cursor_line]));
          }
          editor_dirty = true;
        }
      }
    ...
    // Ctrl+V or F5 (paste)
      ...
      if (g_editor_clipboard && *g_editor_clipboard) {
        ...
        insert_text_at_cursor(&text_buffer, &cursor_line, &cursor_col,
                              g_editor_clipboard);
        editor_dirty = true;
      }
    }
    ...
    // 7) Enter / new line
      ...
      // Notify plugins of the newline insertion
      if (pm) {
        plugins_notify_editor_change(pm, cursor_line, cursor_col + 1, "\n");
      }
    }
    // 8) Backspace
      ...
      if (cursor_col > 0) {
        ...
        // Notify plugins of the character deletion
        if (pm) {
          plugins_notify_editor_change(pm, cursor_line + 1, cursor_col + 1, "");
        }
      } else if (cursor_line > 0) {
        editor_undo_record(&um, &text_buffer, cursor_line, cursor_col,
                           start_line);
        ...
        char *new_line = realloc(text_buffer.lines[cursor_line - 1],
                                 prev_len + curr_len + 1);
        if (!new_line) {
          mvwprintw(notification_window, 1, 2, "Memory allocation error");
          wrefresh(notification_window);
        ...
        text_buffer.lines[cursor_line - 1] = new_line;
        strcat(text_buffer.lines[cursor_line - 1],
               text_buffer.lines[cursor_line]);
        free(text_buffer.lines[cursor_line]);
        ...
        // Notify plugins of the line merge
        if (pm) {
          plugins_notify_editor_change(pm, cursor_line + 1, cursor_col + 1, "");
        }
      }
    ...
    // 10) Printable characters
      ...
      curr_line = new_line;
      memmove(&curr_line[cursor_col + 1], &curr_line[cursor_col],
      ...
      // Notify plugins of the character insertion
      if (pm) {
        char inserted[2] = {(char)ch, '\0'};
        plugins_notify_editor_change(pm, cursor_line + 1, cursor_col, inserted);
      }
    }
    ...
    // Notify plugins if cursor position changed (convert to 1-indexed)
    if (pm &&
        (old_cursor_line != cursor_line || old_cursor_col != cursor_col)) {
      plugins_notify_editor_cursor_move(pm, old_cursor_line + 1,
                                        old_cursor_col + 1, cursor_line + 1,
                                        cursor_col + 1);
    }
  ...
  // We need to clear the main content area (banner_height to LINES -
  // notif_height)
...
bool is_supported_file_type(const char *filename) {
  ...
  if (ext && strcmp(ext, ".js") == 0) {
    ...
    for (size_t i = 0; i < num_supported_mime_types; i++) {
      if (strncmp(mime_type, supported_mime_types[i],
                  strlen(supported_mime_types[i])) == 0) {
        supported = true;
        break;
      }
    }
...
void display_archive_preview(WINDOW *window, const char *file_path,
  ...
  if (!reader) {
    ...
    snprintf(error_msg, sizeof(error_msg),
             "Unable to open archive: %s (errno: %s)", file_path,
             strerror(errno));
  ...
  int adjusted_start_line =
      start_line > header_lines ? start_line - header_lines : 0;
  ...
  while (line_num < max_y - 1 && entry_count < max_entries) {
    ...
    if (ret != 0) {
      if (ret < 0) {
        // Error reading - for single compressed files, don't show technical
        // error
        ...
        if (!is_single_compressed) {
          mvwprintw(window, line_num++, 2,
                    "Archive appears empty (may be corrupted or invalid)");
        }
        // For compressed files, silently fail (they're not really archives)
      }
      break; // Done
    ...
    if (entry.type == ARC_ENTRY_SYMLINK && entry.link_target) {
      snprintf(entry_line, sizeof(entry_line), "%s %-40s %10s -> %s", type_icon,
               entry.path ? entry.path : "(null)", size_str, entry.link_target);
    } else {
      snprintf(entry_line, sizeof(entry_line), "%s %-40s %10s", type_icon,
               entry.path ? entry.path : "(null)", size_str);
    }
  ...
  if (entry_count >= max_entries) {
    mvwprintw(window, line_num++, 2, "... (showing first %d entries)",
              max_entries);
  }
