...
// Ctrl+Shift+Letter codes (starting at 0x2000)
...
static void register_ctrl_arrow_keys_once(void) {
  ...
  // Ctrl+Arrow: Most terminals (xterm, alacritty, kitty, wezterm, iTerm2, GNOME
  // Terminal, etc.)
  ...
  // Ctrl+Shift+Letter: Common terminal escape sequences
  // Format: ESC [ <ascii> ; 6 u (where <ascii> is the letter code, and modifier
  // 6 = Ctrl+Shift) This works in most modern terminals (xterm, alacritty,
  // kitty, wezterm, etc.)
  define_key("\x1b[65;6u", CTRL_SHIFT_A_CODE);
  define_key("\x1b[66;6u", CTRL_SHIFT_B_CODE);
  define_key("\x1b[67;6u", CTRL_SHIFT_C_CODE);
  define_key("\x1b[68;6u", CTRL_SHIFT_D_CODE);
  define_key("\x1b[69;6u", CTRL_SHIFT_E_CODE);
  define_key("\x1b[70;6u", CTRL_SHIFT_F_CODE);
  define_key("\x1b[71;6u", CTRL_SHIFT_G_CODE);
  define_key("\x1b[72;6u", CTRL_SHIFT_H_CODE);
  define_key("\x1b[73;6u", CTRL_SHIFT_I_CODE);
  define_key("\x1b[74;6u", CTRL_SHIFT_J_CODE);
  define_key("\x1b[75;6u", CTRL_SHIFT_K_CODE);
  define_key("\x1b[76;6u", CTRL_SHIFT_L_CODE);
  define_key("\x1b[77;6u", CTRL_SHIFT_M_CODE);
  define_key("\x1b[78;6u", CTRL_SHIFT_N_CODE);
  define_key("\x1b[79;6u", CTRL_SHIFT_O_CODE);
  define_key("\x1b[80;6u", CTRL_SHIFT_P_CODE);
  define_key("\x1b[81;6u", CTRL_SHIFT_Q_CODE);
  define_key("\x1b[82;6u", CTRL_SHIFT_R_CODE);
  define_key("\x1b[83;6u", CTRL_SHIFT_S_CODE);
  define_key("\x1b[84;6u", CTRL_SHIFT_T_CODE);
  define_key("\x1b[85;6u", CTRL_SHIFT_U_CODE);
  define_key("\x1b[86;6u", CTRL_SHIFT_V_CODE);
  define_key("\x1b[87;6u", CTRL_SHIFT_W_CODE);
  define_key("\x1b[88;6u", CTRL_SHIFT_X_CODE);
  define_key("\x1b[89;6u", CTRL_SHIFT_Y_CODE);
  define_key("\x1b[90;6u", CTRL_SHIFT_Z_CODE);
...
size_t num_supported_mime_types =
    sizeof(supported_mime_types) / sizeof(supported_mime_types[0]);
...
void edit_file_in_terminal(WINDOW *window, const char *file_path,
  ...
  // Notify plugins that file is being opened in editor
  if (pm) {
    plugins_notify_editor_open(pm, file_path);
  }
...
bool is_archive_file(const char *filename) {
  ...
  if (strcmp(ext, ".tar") == 0 || strcmp(ext, ".tgz") == 0 ||
      strcmp(ext, ".tbz2") == 0 || strcmp(ext, ".txz") == 0) {
    return true;
  }
  ...
  if (strcmp(ext, ".gz") == 0 || strcmp(ext, ".bz2") == 0 ||
    ...
    // If no .tar, it's a single compressed file (also treat as "archive" for
    // preview)
    return true;
  }
