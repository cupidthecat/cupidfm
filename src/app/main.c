...
int main() {
    ...
    while ((ch = getch()) != kb.key_exit) {
        ...
        if (ch != ERR) {
            ...
            // Save key (Ctrl+S by default) - only works in editor, ignore otherwise
            else if (ch == kb.key_save) {
                // Ctrl+S is handled inside the editor, not in the main file browser
                // Ignore it here to prevent terminal flow control (XOFF) from freezing the UI
                continue;
            }
        }
