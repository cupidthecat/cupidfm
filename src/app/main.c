...
int main() {
  ...
  mainwin =
      newwin(LINES - banner_height - notif_height, COLS, banner_height, 0);
  ...
  dirwin = subwin(mainwin, LINES - banner_height - notif_height,
                  dir_win_width - 1, banner_height, 0);
  box(dirwin, 0, 0);
  wrefresh(dirwin);

  previewwin = subwin(mainwin, LINES - banner_height - notif_height,
                      preview_win_width, banner_height, dir_win_width);
  box(previewwin, 0, 0);
  wrefresh(previewwin);
  ...
  int config_errors =
      load_config_file(&kb, config_path, error_buffer, sizeof(error_buffer));
  ...
  if (config_errors == 0) {
    ...
    if (write_default_config_file(config_path, &kb, error_buffer,
                                  sizeof(error_buffer))) {
      ...
      show_notification(notifwin, "Failed to create default config: %s",
                        error_buffer);
    }
  } else {
    ...
    // Optionally, you can decide whether to proceed with defaults or halt
    // execution For now, we'll proceed with whatever was loaded and keep
    // defaults for invalid entries
  }
  ...
  snprintf(banner_text_buffer, sizeof(banner_text_buffer),
           "Welcome to CupidFM - Press %s to exit",
  ...
  // Load plugins after the UI is drawn so their notifications aren't
  ...
  wtimeout(mainwin,
           INPUT_CHECK_INTERVAL); // Set shorter timeout for input checking
  ...
  int total_scroll_length = (COLS - 2) + (int)strlen(BANNER_TEXT) +
                            (int)strlen(BUILD_INFO) + BANNER_TIME_PREFIX_LEN +
                            BANNER_TIME_LEN + 4;

  int ch;
  while ((ch = cupid_getch_extended(stdscr)) != kb.key_exit) {
    if (state.plugins) {
      plugins_update_context(&state, active_window);
      ...
      // Async plugin UI (prompt/confirm/menu). If a plugin queued one during
      // key handling, run it now (modal), invoke its callback, and then
      // continue handling any requests the callback may have queued
      // (cd/reload/file ops/etc).
      plugins_poll(state.plugins);
      if (plugins_take_quit_request(state.plugins)) {
        break;
      }

      // Plugin-driven navigation/selection (apply even if plugin consumed the
      // key).
      char cd_req[MAX_PATH_LENGTH];
      if (plugins_take_cd_request(state.plugins, cd_req, sizeof(cd_req))) {
        char target[MAX_PATH_LENGTH];
      ...
      if (plugins_take_select_request(state.plugins, sel_req,
                                      sizeof(sel_req))) {
        SIZE idx = (SIZE)-1;
        if (state.search_active) {
          ...
          idx = find_index_by_name_lazy(&state.files, state.current_directory,
                                        &state.dir_window_cas, &state.lazy_load,
                                        sel_req);
        }
      ...
      if (plugins_take_set_search_request(state.plugins, search_req,
                                          sizeof(search_req))) {
        state.select_all_active = false;
        g_select_all_highlight = false;
      ...
      if (plugins_take_parent_dir_request(state.plugins)) {
        navigate_left(&state.current_directory, &state.files,
                      &state.dir_window_cas, &state, &directoryStack);
        state.preview_start_line = 0;
        plugins_update_context(&state, active_window);
        goto input_done;
      }
      ...
      if (plugins_take_enter_dir_request(state.plugins)) {
        navigate_right(&state, &state.current_directory, active_files(&state),
                       &state.dir_window_cas, &directoryStack);
        state.preview_start_line = 0;
        plugins_update_context(&state, active_window);
        goto input_done;
      }
      ...
      if (plugins_take_open_path_request(state.plugins, open_req,
                                         sizeof(open_req))) {
        char target[MAX_PATH_LENGTH];
          ...
          if (stat(target, &st) == 0) {
            if (S_ISDIR(st.st_mode)) {
              ...
              if (state.current_directory) {
                if (state.lazy_load.directory_path)
                  free(state.lazy_load.directory_path);
                state.lazy_load.directory_path =
                    strdup(state.current_directory);
                state.lazy_load.last_load_time = (struct timespec){0};
                reload_directory_lazy(&state.files, state.current_directory,
                                      &state.lazy_load.files_loaded,
                                      &state.lazy_load.total_files);
                ...
                sync_selection_from_active(&state, &state.dir_window_cas);
                state.preview_start_line = 0;
                show_notification(notifwin, "Opened dir: %s",
              ...
              edit_file_in_terminal(previewwin, target, notifwin, &kb,
                                    state.plugins);
              state.preview_start_line = 0;
              redraw_frame_after_edit(&state, dirwin, previewwin, mainwin,
                                      notifwin);
      ...
      if (plugins_take_preview_path_request(state.plugins, preview_req,
                                            sizeof(preview_req))) {
        char target[MAX_PATH_LENGTH];
        ...
        if (!target[0]) {
          ...
          if (stat(target, &st) == 0) {
            ...
            strncpy(state.preview_override_path, target,
                    sizeof(state.preview_override_path) - 1);
            state.preview_override_path[sizeof(state.preview_override_path) -
                                        1] = '\0';
            state.preview_start_line = 0;
      ...
      if (plugins_take_open_selected_request(state.plugins)) {
        ...
        if (view && Vector_len(*view) > 0 && state.dir_window_cas.cursor >= 0 &&
          ...
          FileAttr current_file =
              (FileAttr)view->el[state.dir_window_cas.cursor];
          if (FileAttr_is_dir(current_file)) {
            navigate_right(&state, &state.current_directory, view,
                           &state.dir_window_cas, &directoryStack);
            state.preview_start_line = 0;
          } else {
            ...
            redraw_frame_after_edit(&state, dirwin, previewwin, mainwin,
                                    notifwin);
            show_notification(notifwin, "Editing file: %s",
                              state.selected_entry);
            should_clear_notif = false;
          }
        } else {
      ...
      if (plugins_take_fileop_request(state.plugins, &op)) {
        ...
        if (op.kind == PLUGIN_FILEOP_UNDO || op.kind == PLUGIN_FILEOP_REDO) {
          ...
          if (did) {
            ...
            sync_selection_from_active(&state, &state.dir_window_cas);
            show_notification(notifwin, "%s",
                              (op.kind == PLUGIN_FILEOP_UNDO)
                                  ? "Undone last operation"
                                  : "Redone last operation");
            should_clear_notif = false;
            ok = true;
          } else {
            show_notification(notifwin, "%s",
                              ubuf[0] ? ubuf : "Undo/redo failed");
            should_clear_notif = false;
          }
          ...
          if (full[0]) {
            ...
            if (mkdir(full, 0755) == 0) {
              (void)undo_state_set_single(&state.undo_state, UNDO_OP_CREATE_DIR,
                                          NULL, full);
          ...
          if (full[0]) {
            if (access(full, F_OK) == 0) {
              ...
              if (f) {
                fclose(f);
                (void)undo_state_set_single(&state.undo_state,
                                            UNDO_OP_CREATE_FILE, NULL, full);
          ...
          if (op.count == 1 && op.paths && op.paths[0] && op.arg1[0]) {
            ...
            if (op.arg1[0] == '/') {
              ...
              if (!slash || slash == src) {
                ...
                if (prefix >= sizeof(dirbuf))
                  prefix = sizeof(dirbuf) - 1;
            ...
            if (!dst[0])
              snprintf(err, sizeof(err), "rename: invalid destination");
            else if (access(dst, F_OK) == 0)
              snprintf(err, sizeof(err), "rename: destination exists");
            else {
              (void)ensure_parent_dir_local(dst);
              if (rename(src, dst) == 0) {
                (void)undo_state_set_single(&state.undo_state, UNDO_OP_RENAME,
                                            src, dst);
                show_notification(notifwin, "Renamed: %s", op.arg1);
                should_clear_notif = false;
                ok = true;
              } else {
                snprintf(err, sizeof(err), "rename failed: %s",
                         strerror(errno));
              }
            }
          } else {
          ...
          if (op.count > 0 && op.paths) {
            ...
            if (items && did > 0) {
              if (!undo_state_set_owned(&state.undo_state,
                                        UNDO_OP_DELETE_TO_TRASH, items, did)) {
        ...
        } else if (op.kind == PLUGIN_FILEOP_COPY ||
                   op.kind == PLUGIN_FILEOP_MOVE) {
          if (op.count > 0 && op.paths && op.arg1[0]) {
            ...
            if (stat(dst_dir, &dstst) != 0 || !S_ISDIR(dstst.st_mode)) {
              ...
              for (size_t i = 0; i < op.count; i++) {
                char src[MAX_PATH_LENGTH];
                resolve_path_under_cwd(src, state.current_directory,
                                       op.paths[i]);
                struct stat st;
                if (lstat(src, &st) != 0)
                  continue;
              ...
              if (items && did > 0) {
                UndoOpKind k = (op.kind == PLUGIN_FILEOP_COPY) ? UNDO_OP_COPY
                                                               : UNDO_OP_MOVE;
              ...
              show_notification(
                  notifwin, "%s %zu item(s)",
                  (op.kind == PLUGIN_FILEOP_COPY) ? "Copied" : "Moved", did);
              should_clear_notif = false;
              ok = true;
            }
          } else {
      ...
      if (plugins_take_set_search_mode_request(state.plugins,
                                               &search_mode_req)) {
        ...
        show_notification(notifwin, "Search mode: %s",
                          (search_mode_req == SEARCH_MODE_EXACT)   ? "exact"
                          : (search_mode_req == SEARCH_MODE_REGEX) ? "regex"
                                                                   : "fuzzy");
        should_clear_notif = false;
        plugins_update_context(&state, active_window);
        goto input_done;
    ...
    clock_gettime(CLOCK_MONOTONIC, &current_time);
    long notification_diff =
        (current_time.tv_sec - last_notification_time.tv_sec) * 1000 +
    ...
    if (!notification_hold_active && !should_clear_notif &&
        notification_diff >= NOTIFICATION_TIMEOUT_MS) {
      werase(notifwin);
      wrefresh(notifwin);
    ...
    if (ch != ERR) {
      ...
      if (ch == kb.key_up) {
        if (active_window == DIRECTORY_WIN_ACTIVE) {
          navigate_up(&state.dir_window_cas, active_files(&state),
                      &state.selected_entry, state.current_directory,
      ...
      // 2) DOWN
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE) {
          navigate_down(&state.dir_window_cas, active_files(&state),
                        &state.selected_entry, state.current_directory,
                        state.search_active ? NULL : &state.lazy_load);
          state.preview_start_line = 0;
          werase(notifwin);
          ...
          if (state.preview_override_active) {
            strncpy(file_path, state.preview_override_path,
                    sizeof(file_path) - 1);
      ...
      // 5) TAB (switch active window)
        ...
        werase(notifwin);
        show_notification(notifwin, "Switched to %s window",
                          (active_window == DIRECTORY_WIN_ACTIVE) ? "Directory"
                                                                  : "Preview");
        wrefresh(notifwin);
        should_clear_notif = false;
      }
      ...
      // 7) COPY
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE && state.selected_entry) {
          if (state.select_all_active) {
            ...
            char tmp_path[256];
            snprintf(tmp_path, sizeof(tmp_path), "/tmp/cupidfm_clip_%d",
                     getpid());
            FILE *fp = fopen(tmp_path, "w");
            if (!fp) {
              show_notification(notifwin,
                                "Unable to write clipboard temp file");
              should_clear_notif = false;
              goto input_done;
            }
          ...
          werase(notifwin);
          show_notification(notifwin, "Copied to clipboard: %s",
                            state.selected_entry);
          wrefresh(notifwin);
          should_clear_notif = false;
        }
      ...
      // 8) PASTE
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE) {
          PasteLog plog;
          int pasted = paste_from_clipboard(state.current_directory,
                                            copied_filename, &plog);
          if (pasted <= 0) {
            paste_log_free(&plog);
            werase(notifwin);
          ...
          if (plog.count > 0) {
            ...
            if (items) {
              for (size_t i = 0; i < plog.count; i++) {
                items[i].src =
                    plog.src && plog.src[i] ? strdup(plog.src[i]) : NULL;
                items[i].dst =
                    plog.dst && plog.dst[i] ? strdup(plog.dst[i]) : NULL;
              }
              UndoOpKind kind =
                  (plog.kind == PASTE_KIND_CUT) ? UNDO_OP_MOVE : UNDO_OP_COPY;
              if (!undo_state_set_owned(&state.undo_state, kind, items,
                                        plog.count)) {
                for (size_t i = 0; i < plog.count; i++) {
                  free(items[i].src);
                  free(items[i].dst);
      ...
      // 9) CUT
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE && state.selected_entry) {
          if (state.select_all_active) {
            ...
            int moved = 0;
            UndoItem *undo_items =
                (UndoItem *)calloc(total > 0 ? total : 1, sizeof(UndoItem));
            for (size_t i = 0; i < total; i++) {
              FileAttr fa = (FileAttr)files->el[i];
              const char *name = FileAttr_get_name(fa);
              if (!name || !*name)
              ...
              int n = snprintf(dst_path, sizeof(dst_path), "%s/%s", storage_dir,
                               name);
              if (n < 0 || (size_t)n >= sizeof(dst_path)) {
                continue;
              }
              for (int attempt = 1;
                   access(dst_path, F_OK) == 0 && attempt < 1000; attempt++) {
                n = snprintf(dst_path, sizeof(dst_path), "%s/%s_%d",
                             storage_dir, name, attempt);
                if (n < 0 || (size_t)n >= sizeof(dst_path)) {
                  dst_path[0] = '\0';
                  break;
                }
              ...
              char mv_cmd[2048];
              n = snprintf(mv_cmd, sizeof(mv_cmd), "mv \"%s\" \"%s\"", src_path,
                           dst_path);
              ...
              fprintf(fp, "%d\t%s\t%s\n", FileAttr_is_dir(fa) ? 1 : 0, dst_path,
                      name);
              if (undo_items) {
            ...
            if (undo_items && moved > 0) {
              if (!undo_state_set_owned(&state.undo_state, UNDO_OP_MOVE,
                                        undo_items, (size_t)moved)) {
                for (int i = 0; i < moved; i++) {
                  free(undo_items[i].src);
                  free(undo_items[i].dst);
                }
          ...
          if (storage_path[0] && access(storage_path, F_OK) == 0) {
            (void)undo_state_set_single(&state.undo_state, UNDO_OP_MOVE,
          ...
          werase(notifwin);
          show_notification(notifwin, "Cut to clipboard: %s",
                            state.selected_entry);
          wrefresh(notifwin);
          should_clear_notif = false;
        }
      ...
      // Undo/Redo (Ctrl+Z / Ctrl+Y by default)
        ...
        if (ok) {
          ...
          sync_selection_from_active(&state, &state.dir_window_cas);
          show_notification(notifwin, "%s",
                            (ch == kb.key_undo) ? "Undone last operation"
                                                : "Redone last operation");
        } else {
      ...
      // 10) DELETE
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE && state.selected_entry) {
          if (state.select_all_active) {
            // Bulk delete: delete all items currently visible in the active
            // view
            Vector *files = active_files(&state);
            size_t total = Vector_len(*files);
            ...
            if (undo_items && deleted_count > 0) {
              if (!undo_state_set_owned(&state.undo_state,
                                        UNDO_OP_DELETE_TO_TRASH, undo_items,
            ...
            bool should_delete = false;
            bool delete_result =
                confirm_delete(state.selected_entry, &should_delete);

            if (delete_result && should_delete) {
              char trashed[MAX_PATH_LENGTH] = {0};
              bool deleted_ok =
                  delete_item(full_path, trashed, sizeof(trashed));
              if (deleted_ok) {
                (void)undo_state_set_single(&state.undo_state,
                                            UNDO_OP_DELETE_TO_TRASH, full_path,
                                            trashed);
              }
              char saved_query[MAX_PATH_LENGTH];
              search_before_reload(&state, saved_query);
              ...
              if (deleted_ok) {
                show_notification(notifwin, "Deleted: %s",
                                  state.selected_entry);
              } else {
      ...
      // 11) RENAME
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE) {
          prompt_fuzzy_search(&state, &state.dir_window_cas, notifwin, dirwin,
                              previewwin);
        } else {
      ...
      // 12) CREATE NEW
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE) {
          ...
          if (create_new_file(notifwin, state.current_directory, created_path,
                              sizeof(created_path))) {
            (void)undo_state_set_single(&state.undo_state, UNDO_OP_CREATE_FILE,
                                        NULL, created_path);
          }
          char saved_query[MAX_PATH_LENGTH];
          search_before_reload(&state, saved_query);
      ...
      // 13 CREATE NEW DIR
        ...
        char created_path[MAX_PATH_LENGTH * 2] = {0};
        bool created = create_new_directory(notifwin, state.current_directory,
                                            created_path, sizeof(created_path));
        if (!created) {
          // User canceled or creation failed; don't disturb the current view.
          continue;
        }
        (void)undo_state_set_single(&state.undo_state, UNDO_OP_CREATE_DIR, NULL,
                                    created_path);
        char saved_query[MAX_PATH_LENGTH];
        search_before_reload(&state, saved_query);
        reload_directory(&state.files, state.current_directory);
      ...
      // Quick File Info (Ctrl+T by default)
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE && state.selected_entry) {
          if (state.select_all_active) {
            show_notification(notifwin,
                              "Quick info does not support Select All");
            should_clear_notif = false;
            goto input_done;
          }
          ...
          if (S_ISDIR(st.st_mode))
            strcpy(type_buf, "Directory");
          else if (S_ISREG(st.st_mode))
            strcpy(type_buf, "File");
          else if (S_ISLNK(st.st_mode))
            strcpy(type_buf, "Symlink");
          ...
          if (S_ISDIR(st.st_mode)) {
            ...
            if (dir_size == DIR_SIZE_PENDING) {
              ...
              snprintf(size_buf, sizeof(size_buf), "Virtual FS");
            else if (dir_size == DIR_SIZE_TOO_LARGE)
              snprintf(size_buf, sizeof(size_buf), "Too large");
            else if (dir_size == DIR_SIZE_PERMISSION_DENIED)
              snprintf(size_buf, sizeof(size_buf), "Permission denied");
            else if (dir_size < 0)
              snprintf(size_buf, sizeof(size_buf), "Error");
            else {
              char tmp[32];
          ...
          char perm_buf[16];
          snprintf(perm_buf, sizeof(perm_buf), "%04o",
                   (unsigned)(st.st_mode & 07777));

          char mtime_buf[64];
          struct tm tm_local;
          localtime_r(&st.st_mtime, &tm_local);
          strftime(mtime_buf, sizeof(mtime_buf), "%Y-%m-%d %H:%M:%S",
                   &tm_local);
          ...
          if (S_ISLNK(st.st_mode) && link_target[0]) {
            show_popup("File Info",
                       ...
                       "MIME: %s\n"
                       "MTime: %s",
                       state.selected_entry, type_buf, file_path, link_target,
                       size_buf, perm_buf, mime_buf, mtime_buf);
          } else {
            show_popup("File Info",
                       "Name: %s\n"
                       "Type: %s\n"
                       ...
                       "MIME: %s\n"
                       "MTime: %s",
                       state.selected_entry, type_buf, file_path, size_buf,
                       perm_buf, mime_buf, mtime_buf);
          }
        }
      }
      ...
      // File Permissions (Ctrl+P by default)
        ...
        if (active_window == DIRECTORY_WIN_ACTIVE && state.selected_entry) {
          if (state.select_all_active) {
            show_notification(notifwin,
                              "Permissions does not support Select All");
            should_clear_notif = false;
            goto input_done;
          }
          char full_path[MAX_PATH_LENGTH];
  ...
  input_done:
    ...
    // Keep plugin context up-to-date after CupidFM handled input, and fire
    // change hooks.
    if (state.plugins) {
      plugins_update_context(&state, active_window);
    }
    ...
    // Redraw windows
    draw_directory_window(dirwin, state.current_directory, active_files(&state),
                          &state.dir_window_cas);

    if (state.preview_override_active) {
      draw_preview_window_path(previewwin, state.preview_override_path, NULL,
                               state.preview_start_line);
    } else {
      draw_preview_window(previewwin, state.current_directory,
                          state.selected_entry, state.preview_start_line);
    }

    // Removed: duplicate active-window cursor highlighting. Use render-only
    // highlights in draw_directory_window.
    ...
    if (active_window == DIRECTORY_WIN_ACTIVE) {
      ...
      if (view && Vector_len(*view) > 0 && state.dir_window_cas.cursor >= 0 &&
        ...
        mvwprintw(
            dirwin,
            state.dir_window_cas.cursor - state.dir_window_cas.start + 1, 1,
            "%s",
  ...
  // `search_files` is a shallow view into `files`, so only free its backing
  // array.
