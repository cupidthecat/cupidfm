...
static void keycode_to_config_string_local(int keycode, char *buf,
                                           size_t buf_size) {
  if (!buf || buf_size == 0)
    return;
  if (keycode >= KEY_F(1) && keycode <= KEY_F(63)) {
...
static int nf_fm_cache_set(cs_vm *vm, void *ud, int argc, const cs_value *argv,
  ...
  char *encoded_val =
      (argv[1].type == CS_T_NIL) ? NULL : encode_cache_value(&argv[1]);
...
static void register_fm_api(PluginManager *pm, cs_vm *vm) {
  ...
  cs_register_native(vm, "fm.editor_get_selection", nf_fm_editor_get_selection,
                     pm);
  cs_register_native(vm, "fm.editor_insert_text", nf_fm_editor_insert_text, pm);
  cs_register_native(vm, "fm.editor_replace_text", nf_fm_editor_replace_text,
                     pm);
  cs_register_native(vm, "fm.editor_delete_range", nf_fm_editor_delete_range,
                     pm);
